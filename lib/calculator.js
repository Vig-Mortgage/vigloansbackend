...
const CALC_CONFIG = {
  ...
  // USDA Guaranteed: guarantee fee de 1% (financiable) + annual fee de 0.35%
  // del balance de principal, prorrateado mensual. Ambos vigentes FY2026.
  // Fuente: USDA RD / HUD-equivalente; revisado 2026-08.
  usda: { upfrontRate: 0.01, annualRate: 0.0035 },
...
const FinancialCalc = {
  ...
  /** Guarantee fee upfront de USDA (1%), financiable en el préstamo. */
  calculateUSDAUpfront(loanBase) {
    return Math.round(loanBase * CALC_CONFIG.usda.upfrontRate);
  },

  /**
   * Annual fee de USDA prorrateado mensual. La base es el balance de principal
   * — es decir, el préstamo total INCLUYENDO el guarantee fee financiado.
   */
  calculateUSDAFee(loanBalance) {
    return (loanBalance * CALC_CONFIG.usda.annualRate) / 12;
  },
  ...
  /**
   * PMI mensual. Se cotiza como % anual del **monto financiado**, no del valor
   * de la propiedad. El legacy usaba el valor de la propiedad, lo que
   * sobrestimaba la prima. Corregido 2026-08.
   */
  calculateMonthlyPMI(loanAmount, pmiRate) {
...
function computeQuote(input) {
  ...
  if (loanType === 'FHA') {
    ...
    // La exención (discapacidad relacionada al servicio) es opcional y solo
    // aplica a VA: el cliente la enciende/apaga; por defecto se cobra el fee.
    ...
    upfrontFee = FinancialCalc.calculateUSDAUpfront(loanBase);
  }
  ...
  if (loanType === 'FHA') {
    ...
    // El MIP anual se calcula sobre el Base Loan Amount (el préstamo ANTES de
    // financiar el UFMIP), por definición de HUD. El legacy usaba
    // `precio - UFMIP`, que no corresponde a ninguna base real. Corregido 2026-08.
    monthlyMIP = FinancialCalc.calculateMonthlyMIP(loanBase, rate);
    mipDurationForTable = duration;
  } else if (loanType === 'USDA') {
    monthlyMIP = FinancialCalc.calculateUSDAFee(totalLoanAmount);
  } else if (loanType === 'CONV') {
    if (ltv > 80) {
      const rate = FinancialCalc.calculatePMIRate(creditScore, dpPercent, term);
      monthlyMIP = FinancialCalc.calculateMonthlyPMI(loanBase, rate);
    }
  }
