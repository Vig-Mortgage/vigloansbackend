...
const SecretName = Object.freeze({
  ...
  // Los dos que ya usa el PHP legacy, con sus nombres tal cual estan creados
  // en Secrets Manager. `twillio` lleva la errata de dos eles: se respeta
  // porque el legacy sigue vivo leyendo ese mismo secreto, y renombrarlo
  // romperia los envios de produccion.
  TWILIO: 'twillio',
  WHATSAPP: 'whatsapp',
});
...
function createSecretsProvider({
  ...
  return {
    ...
    getTwilioSecrets: () => getSecret(SecretName.TWILIO),
    getWhatsappSecrets: () => getSecret(SecretName.WHATSAPP),
    /** Solo para tests. */
