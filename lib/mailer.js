...
function buildTransportOptions(raw) {
  ...
  // TLS implicito si lo pide el puerto, la bandera, o el `MAIL_ENCRYPTION` del
  // secreto.
  //
  // El secreto `Mail` de VIG trae MAIL_PORT=443 y MAIL_ENCRYPTION=ssl, y
  // mail.smtp2go.com habla TLS implicito en 443 (verificado: el banner 220
  // llega DESPUES del handshake, no antes). Sin esta linea intentariamos
  // STARTTLS contra un socket que ya esta cifrado y el envio fallaria.
  //
  // Nota de contexto: el secreto llevaba la configuracion correcta desde
  // siempre; el cliente casero de `app.js` la ignoraba y clavaba el 2525 en
  // claro.
  const cifradoImplicito = String(
    firstDefined(raw.encryption, raw.MAIL_ENCRYPTION) ?? ''
  ).trim().toLowerCase();
  const secure =
    port === IMPLICIT_TLS_PORT ||
    raw.secure === true ||
    cifradoImplicito === 'ssl' ||
    cifradoImplicito === 'tls';
