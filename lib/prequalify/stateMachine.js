 ...
 *
 * Contexto historico: `mailingAddress` esta en el HTML legacy (`index.php`,
 * `fieldsetMailingAddress`) y tiene endpoint propio
 * (`accionMailingAddress.php`), pero **ningun JS lo muestra jamas**: en el
 * wizard legacy es inalcanzable, asi que el sistema nunca capturo esos datos.
 * La condicion de abajo no se porta del legacy (no existia) sino de la URLA.
 *
 * `previousAddress` existio aqui y se ELIMINO el 2026-08-07: Salesforce no
 * tiene donde guardar la direccion (ver la nota en `salesforceMapper.js`).
 */
 ...
 *
 * El legacy no tiene numero para el paso que nunca alcanza (mailingAddress)
 * ni para el co-deudor.
 */
...
function nextStep(current, ctx = {}) {
  ...
  //
  // Sin esto, `nextStep(START)` devolvia siempre `otpVerify`, porque
  // `otpVerify` aplica a todo lead. Pero el orden de EJECUCION real no es el
  // de `STEP_ORDER`: el cliente verifica los codigos primero —ahi nace el lead
  // y se emite la sesion— y solo despues manda `start`. Asi que al completar
  // `start` el backend contestaba "ahora toca otpVerify", el cliente releia el
  // lead, le decia que reanudara en el primer paso, y el wizard volvia al
  // principio en bucle. Sintoma reportado: "cuando valido el codigo, vuelve al
  // primer paso".
  //
  // "El siguiente paso" solo puede significar el siguiente que APLICA y que
  // FALTA. Un paso hecho no vuelve a tocar.
  const hechos = new Set(Array.isArray(ctx.completedSteps) ? ctx.completedSteps : []);

  for (let i = stepIndex(current) + 1; i < STEP_ORDER.length; i += 1) {
    const candidate = STEP_ORDER[i];
    if (candidate === Step.DONE) return Step.DONE;
    if (appliesTo(candidate, ctx) && !hechos.has(candidate)) return candidate;
  }
  return Step.DONE;
}
