...
function createOtpService({
  ...
  /**
   * @param {object} params
   * @param {string} params.channel **identidad** del reto. Junto al destino
   *   forma la clave del almacen. Un telefono debe usar siempre el mismo valor
   *   ('phone') aunque el codigo se entregue por SMS una vez y por WhatsApp la
   *   siguiente: si no, reenviar por otra via crearia un reto distinto y el
   *   codigo anterior dejaria de verificar.
   * @param {string} params.destination
   * @param {string} [params.deliveryChannel] por donde se entrega. Por defecto,
   *   el mismo `channel`.
   * @param {string} [params.locale]
   */
  async function requestCode({ channel, destination, deliveryChannel, locale }) {
