...
/**
 * Base de todo campo de texto.
 *
 * `required_error` explicito porque zod responde "Required" en ingles cuando el
 * campo falta, y ese texto acaba en la pantalla del solicitante. El contrato
 * debe venir traducido de origen: si no, cada cliente (web, Flutter) tiene que
 * mantener su propia tabla de traducciones del mismo mensaje.
 */
...
/**
 * Telefono de Puerto Rico o Estados Unidos: 10 digitos, con o sin separadores.
 *
 * PR usa el mismo plan de numeracion que EEUU (NANP, codigo +1) con los
 * codigos de area 787 y 939, asi que la misma validacion cubre los dos y no
 * hace falta distinguir. Se acepta el prefijo 1 opcional.
 *
 * Se normaliza a 10 digitos para que el dedupe por telefono (Tarea B5) compare
 ...
 */
/**
 * Area 666: la SSA no la emite NUNCA, y por eso Experian la usa para sus
 * personas de prueba (las tres de `data_score.json` empiezan por 666).
 *
 * Eso crea un choque: la validacion correcta para produccion hace imposible
 * probar la consulta de credito desde la interfaz. El formulario responde "SSN
 * invalido" antes de que la peticion llegue a Experian, asi que el pull solo se
 * podia ejercitar por script.
 *
 * La excepcion va atada a `EXPERIAN_USE_UAT`, que es la MISMA bandera que
 * decide a que entorno se pide el reporte. Las dos cosas no pueden divergir:
 *
 *   EXPERIAN_USE_UAT=true   -> Experian es UAT, y ahi un SSN 666 es lo unico
 *                              que tiene sentido. Se acepta.
 *   EXPERIAN_USE_UAT=false  -> Experian es produccion. Se rechaza, como debe.
 *
 * Es el patron que exige el CLAUDE.md del repo: "los mocks van tras un flag de
 * entorno, jamas en el flujo real".
 *
 * Se lee en cada validacion y no al cargar el modulo, para que un test pueda
 * ejercitar los dos comportamientos.
 *
 * SOLO se relaja el area 666. `000` y las que empiezan por 9 siguen prohibidas
 * en los dos entornos: no son rangos de prueba de nadie, son basura.
 */
function aceptaSsnDePruebaDeExperian() {
  return process.env.EXPERIAN_USE_UAT === 'true';
}
...
const ssn = trimmed
  ...
  .refine((digits) => {
    ...
    if (area === '000' || area[0] === '9') return false;
    if (area === '666') return aceptaSsnDePruebaDeExperian();
...
const citizenship = z.enum(
  ['U.S. Citizen', 'Permanent Resident Alien', 'Non-Permanent Resident Alien'],
  requerido('Selecciona tu ciudadania')
);
const maritalStatus = z.enum(
  ['Married', 'Single', 'Unmarried'],
  requerido('Selecciona tu estado civil')
);
const housing = z.enum(
  ['No primary housing expense', 'Own', 'Rent'],
  requerido('Selecciona el tipo de vivienda')
);
const typeOfCredit = z.enum(
  ['IndividualCredit', 'CoborrowerCredit'],
  requerido('Selecciona el tipo de credito')
);
const incomeFrequency = z.enum(
  ['Biweekly', 'Semimonthly', 'Weekly'],
  requerido('Selecciona la frecuencia de pago')
);
const loanPurpose = z.enum(
  ['Compra', 'Refinancia'],
  requerido('Indica si es compra o refinanciamiento')
);
...
/**
 * `POST /prequalify/otp`: pide los codigos.
 *
 * SE VERIFICAN LOS DOS CANALES, no uno u otro. Se envia un codigo al telefono
 * y otro al correo, y la sesion no se emite hasta que ambos se confirman. Es
 * mas friccion, y es deliberada: aqui se captura un SSN y se jala credito, asi
 * que conviene saber que quien lo pide controla las dos vias de contacto. Un
 * correo tipeado mal, o de otra persona, no deberia poder llegar hasta ahi.
 *
 * `phoneChannel` elige por donde va el del telefono; el del correo va por
 * correo, obviamente.
 */
const otpRequestSchema = z.object({
  email,
  phone,
  phoneChannel: z.enum(['sms', 'whatsapp'], requerido('Elige SMS o WhatsApp')).default('sms'),
});

/**
 * `otpVerify`: canjea AMBOS codigos por un token de sesion.
 *
 * Los dos van en la misma peticion a proposito. La alternativa —verificar uno,
 * emitir un token parcial, verificar el otro— obliga a mantener un estado
 * intermedio y a un token que casi puede hacer cosas. Con los dos juntos no
 * hay estado a medias: o entran los dos, o no hay sesion.
 */
const otpVerifySchema = z.object({
  email,
  phone,
  emailCode: trimmed.regex(/^\d{6}$/, 'El codigo del correo debe tener 6 digitos'),
  phoneCode: trimmed.regex(/^\d{6}$/, 'El codigo del telefono debe tener 6 digitos'),
  // El nombre viaja AQUI porque es donde nace el lead.
  //
  // La org tiene una regla de validacion —"Debe Ingresar el Nombre del
  // Lead"— que rechaza un insert sin nombre. Antes este endpoint creaba el
  // lead con solo email y telefono y dejaba el nombre para el paso `start`,
  // asi que Salesforce devolvia 400 y el usuario veia "No pudimos completar
  // la operacion" justo despues de teclear sus dos codigos.
  //
  // Los dos clientes ya tienen estos datos en este punto: recogen la
  // identidad en la primera pantalla y solo despues piden el codigo. No se
  // pide nada nuevo al usuario.
  //
  // Son REQUERIDOS y no opcionales a proposito: sin ellos la creacion del
  // lead es imposible, y un 400 que dice que falta el nombre es mas util que
  // un 502 generico del proveedor.
  firstName: personName,
  lastName: personName,
});
...
/**
 * `credit-check`: el SSN vuelve a viajar en esta peticion.
 *
 * No se puede recuperar de Salesforce —se escribe pero NUNCA se relee, ver
 * `SENSITIVE_LEAD_FIELDS` en `salesforceMapper.js`— y el backend no lo guarda
 * entre pasos. Lo tiene el cliente, que es quien se lo pidio al usuario, y lo
 * reenvia aqui. Es lo mismo que hace el legacy (`scripts.js:1010-1029`).
 *
 * El coste es que el SSN viaja dos veces y el cliente ha de conservarlo entre
 * el paso `personal` y este. La alternativa era guardarlo server-side, que
 * exige decidir donde vive y garantizar el borrado.
 */
const creditCheckSchema = z.object({ ssn });
