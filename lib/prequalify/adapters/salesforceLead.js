...
function createSalesforceLeadAdapter({
   ...
   *
   * Se CREA, no se actualiza, por fidelidad al legacy: `accionIncome.php:184` y
   * `accionEmployment_SelfEmployment.php:312` llaman a `create_objeto()`, que es
   * un POST plano — no consultan si ya existe ni hacen PATCH. Verificado en el
   * PHP, no supuesto.
   *
   * CONSECUENCIA: rehacer un paso deja DOS registros hijos colgando del mismo
   * lead, y un asesor ve dos empleos donde hay uno. Pasa con un reintento por
   * red, con el boton de atras, o con cualquier reenvio. La org de produccion
   * ya arrastra estos duplicados desde el legacy.
   *
   * Se mantiene el comportamiento porque cambiarlo altera lo que la org recibe
   * y eso es decision de negocio, no tecnica. Si se decide corregirlo, la
   * forma es consultar por `Lead__c` y hacer PATCH del registro existente en
   * vez de POST. Anotado en `03_PLAN_CIERRE_PRECUALIFICACION.md`.
   */
  ...
  async function updateLead(leadId, fields, { step } = {}) {
    ...
    const campos = mapper.toLeadFields(mapper.toApiModelForStep(step, resto));
