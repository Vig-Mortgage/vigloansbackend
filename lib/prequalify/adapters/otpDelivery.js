...
const { createMailer, ALLOWED_MESSAGE_FIELDS } = require('../../mailer');
const {
  Template,
  buildEmailMessage,
  buildShortMessage,
...
/**
 * Se queda con los campos que `sendMail` acepta.
 *
 * Se deriva de `ALLOWED_MESSAGE_FIELDS` del propio mailer y no de una lista
 * copiada aqui: si el mailer cambia lo que admite, esto lo sigue solo.
 */
function soloCamposDeCorreo(mensaje) {
  const salida = {};
  for (const campo of ALLOWED_MESSAGE_FIELDS) {
    if (mensaje[campo] !== undefined) salida[campo] = mensaje[campo];
  }
  return salida;
}
...
function createOtpDeliveryAdapter({
  ...
  async function send({ channel, destination, code, ttlSeconds, locale }) {
    ...
    if (channel === 'email') {
      ...
      // `buildEmailMessage` devuelve el mensaje MAS metadatos (`template`,
      // `locale`, `containsSecret`) que sirven para decidir y auditar, no para
      // enviar. `sendMail` rechaza cualquier campo que no conozca —a
      // proposito, para que nadie cuele cabeceras— asi que se proyecta sobre
      // los permitidos en vez de pasarle el objeto entero.
      const info = await mailer.sendMail(soloCamposDeCorreo(mensaje));
